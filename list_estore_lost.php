...
$pestore_lost="";
if(isset($_POST["estore_lost"])){
	if(!empty($_POST["estore_lost"])){
		$pestore_lost=strtolower(filter_input(INPUT_POST, 'estore_lost', FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH));
	}else{
	}
}else{
}
...
if($ceik!==""){
	$sql="UPDATE ".$GLOBALS['bg_table']." SET ".$GLOBALS['bg_datechecked_scrap']."=0, ".$GLOBALS['bg_estore']."='', ".$GLOBALS['bg_estorev2']."='', ".$GLOBALS['bg_estorev3']."='', ".$GLOBALS['bg_facebook']."='', ".$GLOBALS['bg_twitter']."='', ".$GLOBALS['bg_linkedin']."='', ".$GLOBALS['bg_google']."='', ".$GLOBALS['bg_youtube']."='', ".$GLOBALS['bg_pinterest']."='', ".$GLOBALS['bg_instagram']."='', ".$GLOBALS['bg_etitle']."='', ".$GLOBALS['bg_edescription']."='', ".$GLOBALS['bg_ekeywords']."='', ".$GLOBALS['bg_etitle']."='', ".$GLOBALS['bg_edescription']."='', ".$GLOBALS['bg_ekeywords']."='', ".$GLOBALS['bg_etitlev2']."='', ".$GLOBALS['bg_edescriptionv2']."='', ".$GLOBALS['bg_ekeywordsv2']."='', ".$GLOBALS['bg_etitlev3']."='', ".$GLOBALS['bg_edescriptionv3']."='', ".$GLOBALS['bg_ekeywordsv3']."='' WHERE ".$GLOBALS['bg_EIK']."='".$ceik."';";
...
if($peik!==""){
	$sql="UPDATE ".$GLOBALS['bg_table']." SET ".$GLOBALS['bg_estore_lost']."='".$pestore_lost."' WHERE ".$GLOBALS['bg_EIK']."='".$peik."';";
	$result = mysql_query($sql);
}

$sql="SELECT * FROM ".$GLOBALS['bg_table']." WHERE ".$GLOBALS['bg_url_final_stop']."<>'' and ".$GLOBALS['bg_estore']."='' and ".$GLOBALS['bg_estorev2']."='' and ".$GLOBALS['bg_estorev3']."='' LIMIT ".($n*10).", ".($l*10)."";
$result = mysql_query($sql);
if($result){
	$j=0;
	...
	while ($row = mysql_fetch_assoc($result)) {
			if (strpos($row[$GLOBALS['bg_url_final']], 'http') !== false) {
				$url=$row[$GLOBALS['bg_url_final']];
			}else{
				$url="http://".$row[$GLOBALS['bg_url_final']];				
			}
		$j++;
		if($j % 10 === 0){
		$i++;
...
//		echo urlpost($row[$GLOBALS['bg_EIK']]);
		...
		echo "</td>";
		echo "<td>".estorepost_lost($row[$GLOBALS['bg_EIK']],$row[$GLOBALS['bg_estore_lost']])."<a href=\"".$row[$GLOBALS['bg_estore_lost']]."\" target=\"blank\">".$row[$GLOBALS['bg_estore_lost']]."</a></td>";
...
function estorepost_lost($eik,$v){
	global $bg_navigation;
	$r="";
//	if($v!==""){
			$r="<form method=\"post\" action=\"".$_SERVER["REQUEST_URI"]."\">
  <input type=\"hidden\" name=\"eik\" value=\"".$eik."\">
  <input type=\"text\" name=\"estore_lost\" value=\"".$v."\" size=\"30\">
  <input type=\"submit\" value=\"".$bg_navigation[17]."\">
</form>";
//	}
	return $r;
