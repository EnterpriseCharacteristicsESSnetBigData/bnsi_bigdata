...
$arrk=array();
	$i1=0;
	$z1=0;
$sql="SELECT DISTINCT ".$GLOBALS['bg_nace3'].", count(".$GLOBALS['bg_nace3'].") as kid, SUM(".$GLOBALS['bg_employee'].") as zaeti FROM ".$GLOBALS['bg_table']." GROUP BY ".$GLOBALS['bg_nace3']."";
...
if($result){
	while ($row = mysql_fetch_assoc($result)) {
		...
		$e1+=$row["estore"];
		$z1+=$row["zaeti"];
		$arrk[]=$row[$GLOBALS['bg_nace3']];
		$arr[$row[$GLOBALS['bg_nace3']]]["t"]["k"]=$row["kid"];
		$arr[$row[$GLOBALS['bg_nace3']]]["t"]["z"]=$row["zaeti"];
	}
}
//URL/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$i2=0;
	$z2=0;
$sql="SELECT DISTINCT ".$GLOBALS['bg_nace3'].", count(".$GLOBALS['bg_nace3'].") as kid, SUM(".$GLOBALS['bg_employee'].") as zaeti FROM ".$GLOBALS['bg_table']." WHERE ".$GLOBALS['bg_url_final_stop']."<>'' GROUP BY ".$GLOBALS['bg_nace3']."";
$result = mysql_query($sql);
if($result){
	while ($row = mysql_fetch_assoc($result)) {
		$i2+=$row["kid"];
		$e2+=$row["estore"];
		$z2+=$row["zaeti"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["k"]=$row["kid"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["e"]=$row["estore"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["z"]=$row["zaeti"];
	}
}
//E-store/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$e3=0;
	$z3=0;
$sql="SELECT DISTINCT ".$GLOBALS['bg_nace3'].", count(".$GLOBALS['bg_estore_final_stop'].") as estore, SUM(".$GLOBALS['bg_employee'].") as zaeti FROM ".$GLOBALS['bg_table']." WHERE ".$GLOBALS['bg_estore_final_stop']."<>'' GROUP BY ".$GLOBALS['bg_nace3']."";
$result = mysql_query($sql);
if($result){
	while ($row = mysql_fetch_assoc($result)) {
		$e3+=$row["estore"];
		$z3+=$row["zaeti"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["e"]=$row["estore"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["z3"]=$row["zaeti"];
	}
}
//Social Media/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$e4=0;
	$z4=0;
$sql="SELECT DISTINCT ".$GLOBALS['bg_nace3'].", count(".$GLOBALS['bg_nace3'].") as socialmedia, SUM(".$GLOBALS['bg_employee'].") as zaeti FROM ".$GLOBALS['bg_table']." WHERE ".$GLOBALS['bg_facebook']."<>'' OR ".$GLOBALS['bg_twitter']."<>'' OR ".$GLOBALS['bg_linkedin']."<>'' OR ".$GLOBALS['bg_google']."<>'' OR ".$GLOBALS['bg_youtube']."<>'' OR ".$GLOBALS['bg_pinterest']."<>'' OR ".$GLOBALS['bg_instagram']."<>'' GROUP BY ".$GLOBALS['bg_nace3']."";
$result = mysql_query($sql);
if($result){
	while ($row = mysql_fetch_assoc($result)) {
		$e4+=$row["socialmedia"];
		$z4+=$row["zaeti"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["s"]=$row["socialmedia"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["z4"]=$row["zaeti"];
	}
}
//Jobs Advertising/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	$e5=0;
	$z5=0;
$sql="SELECT DISTINCT ".$GLOBALS['bg_nace3'].", count(".$GLOBALS['bg_nace3'].") as jobs, SUM(".$GLOBALS['bg_employee'].") as zaeti FROM ".$GLOBALS['bg_table']." WHERE ".$GLOBALS['bg_jobs_final_stop']."<>'' GROUP BY ".$GLOBALS['bg_nace3']."";
$result = mysql_query($sql);
if($result){
	while ($row = mysql_fetch_assoc($result)) {
		$e5+=$row["jobs"];
		$z5+=$row["zaeti"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["j"]=$row["jobs"];
		$arr[$row[$GLOBALS['bg_nace3']]]["u"]["z5"]=$row["zaeti"];
	}
}
