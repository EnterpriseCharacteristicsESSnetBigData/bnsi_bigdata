...
$pjobs_lost="";
if(isset($_POST["jobs_lost"])){
	if(!empty($_POST["jobs_lost"])){
		$pjobs_lost=strtolower(filter_input(INPUT_POST, 'jobs_lost', FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH));
	}else{
	}
}else{
}
...
if($ceik!==""){
	$sql="UPDATE ".$GLOBALS['bg_table']." SET ".$GLOBALS['bg_datechecked_scrap']."=0, ".$GLOBALS['bg_jobs']."='', ".$GLOBALS['bg_jobsv2']."='', ".$GLOBALS['bg_jobsv3']."='', ".$GLOBALS['bg_jtitle']."='', ".$GLOBALS['bg_jdescription']."='', ".$GLOBALS['bg_jkeywords']."='', ".$GLOBALS['bg_jtitle']."='', ".$GLOBALS['bg_jdescription']."='', ".$GLOBALS['bg_jkeywords']."='', ".$GLOBALS['bg_jtitlev2']."='', ".$GLOBALS['bg_jdescriptionv2']."='', ".$GLOBALS['bg_jkeywordsv2']."='', ".$GLOBALS['bg_jtitlev3']."='', ".$GLOBALS['bg_jdescriptionv3']."='', ".$GLOBALS['bg_jkeywordsv3']."='' WHERE ".$GLOBALS['bg_EIK']."='".$ceik."';";
...
if($peik!==""){
	$sql="UPDATE ".$GLOBALS['bg_table']." SET ".$GLOBALS['bg_jobs_lost']."='".$pjobs_lost."' WHERE ".$GLOBALS['bg_EIK']."='".$peik."';";
	$result = mysql_query($sql);
}

$sql="SELECT * FROM ".$GLOBALS['bg_table']." WHERE ".$GLOBALS['bg_url_final_stop']."<>'' and ".$GLOBALS['bg_jobs']."='' and ".$GLOBALS['bg_jobsv2']."='' and ".$GLOBALS['bg_jobsv3']."='' LIMIT ".($n*10).", ".($l*10)."";
$result = mysql_query($sql);
if($result){
	$j=0;
...
//		echo urlpost($row[$GLOBALS['bg_EIK']]);
		...
		echo "</td>";
		echo "<td>".jobspost_lost($row[$GLOBALS['bg_EIK']],$row[$GLOBALS['bg_jobs_lost']])."<a href=\"".$row[$GLOBALS['bg_jobs_lost']]."\" target=\"blank\">".$row[$GLOBALS['bg_jobs_lost']]."</a></td>";
...
function jobspost_lost($eik,$v){
	global $bg_navigation;
	$r="";
//	if($v!==""){
			$r="<form method=\"post\" action=\"".$_SERVER["REQUEST_URI"]."\">
  <input type=\"hidden\" name=\"eik\" value=\"".$eik."\">
  <input type=\"text\" name=\"jobs_lost\" value=\"".$v."\" size=\"30\">
  <input type=\"submit\" value=\"".$bg_navigation[17]."\">
</form>";
//	}
	return $r;
